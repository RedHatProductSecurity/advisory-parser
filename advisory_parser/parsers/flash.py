...
DESCRIPTION_TEMPLATE = (
    "Adobe Security Bulletin {advisory_id} for Adobe Flash "
...
def parse_flash_advisory(url):
    ...
    for vuln_impact, group_1 in groupby(sorted(vuln_data), lambda x: x[0]):
        ...
        highest_severity = sorted([entry[2].lower() for entry in data], key=SEVERITY_ORDER.index)[0]
        ...
        summary = "flash-plugin: {} {} ({})".format(
            vuln_impact, "vulnerability" if len(all_cves) == 1 else "vulnerabilities", advisory_id
        )
        ...
        flaws.append(
            Flaw(
                ...
                cvss3=CVSS_MAP.get(
                    vuln_impact.lower(), "8.8/CVSS:3.0/AV:N/AC:L/PR:N/UI:R/S:U/C:H/I:H/A:H"
                ),
                advisory_id=advisory_id,
            )
        )
